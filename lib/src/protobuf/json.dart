...
Map<String, dynamic> _writeToJsonMap(_FieldSet fs, {bool useInserStm = false}) {
    ...
    switch (baseType) {
      case 2097152:
        if (fieldValue is Struct) {
          final item = fieldValue as Struct;
          final jsonString = jsonEncode(item.toProto3Json());
          return useInserStm ? '\'$jsonString\'' : jsonString;
        } else {
          return fieldValue.writeToJsonMap(useInserStm: useInserStm);
        }
      ...
      case PbFieldType._STRING_BIT:
        return useInserStm ? '\'$fieldValue\'' : fieldValue;
  ...
  for (var fi in fs._infosSortedByTag) {
    ...
    if (value == null || (value is List && value.isEmpty)) {
      ...
      result['${fi.protoName}'] = 'default';
      continue;
    }
    // if (_isMapField(fi.type)) {
    //   result['${fi.protoName}'] = _writeMap(value, fi);
    //   continue;
    // }
    //  print(value);
    // if (_isMapField(fi.type)) {
    ...
    if (value.runtimeType.toString().trim() == 'Timestamp') {
      var fecha = DateTime.fromMicrosecondsSinceEpoch(
          value.seconds.toInt() * Duration.microsecondsPerSecond +
              value.nanos ~/ 1000,
          isUtc: false);

      result['${fi.protoName}'] = '\'${fecha.toString()}\'';
    } else {
      result['${fi.protoName}'] = convertToMap(value, fi.type);
    }
